...
from .crypto_utils import (
    rsa_generate_2048_pem_pair,
    encrypt_with_vault_secret,
    decrypt_with_vault_secret,
)
...
@router.post(
    "/google"
)  # Note: Retorno genérico (dict) pois pode ser Token ou Status 2FA
def auth_google(data: dict):
    ...
    try:
        ...
        flow = Flow.from_client_config(
            client_config, scopes=None, redirect_uri=os.getenv("GOOGLE_REDIRECT_URI")
        )
        ...
        if not email or not google_sub or not username:
            raise HTTPException(
                status_code=400, detail="Erro Google: E-mail não encontrado."
            )

        print(
            f"--- [GOOGLE-AUTH] Email: {email}, Google Sub: {google_sub}, Username calculado: '{username}'"
        )
        ...
        if not user:
            ...
            new_user = {
                ...
                "encrypted_private_key_ciphertext": key_data[
                    "encrypted_private_key_ciphertext"
                ],
            }
            print(f"--- [GOOGLE-AUTH] Criando novo usuário: {new_user}")
            db.users.insert_one(new_user)
            user = db.users.find_one(
                {"username": username}
            )  # Recarrega o usuário criado
            print(f"--- [GOOGLE-AUTH] Usuário criado e recarregado: {user}")
        else:
            print(
                f"--- [GOOGLE-AUTH] Usuário já existe. Username no banco: '{user.get('username')}'"
            )
            # Usar o username do banco, não o calculado
        ...
        print(f"--- [GOOGLE-AUTH] Username para salvar OTP: '{username}'")
        ...
        print(f"--- [GOOGLE-AUTH] OTP salvo para username: '{username}'")
        ...
        print(f"--- [GOOGLE-AUTH] Enviando para frontend - username: '{username}'")
        return {
            "require_2fa": True,
            "username": username,
...
def verify_2fa(data: dict):
    ...
    db = get_db()
    print(f"--- [VERIFY-2FA] Procurando usuário com username: '{username}'")

    user = db.users.find_one({"username": username})

    if not user:
        print(f"--- [VERIFY-2FA] ❌ Usuário NÃO encontrado para username: '{username}'")
        raise HTTPException(404, f"Usuário '{username}' não encontrado.")

    print(f"--- [VERIFY-2FA] ✅ Usuário encontrado: {user.get('username')}")
    ...
    db.users.update_one(
        {"username": username}, {"$unset": {"otp_code": "", "otp_expires": ""}}
    )
