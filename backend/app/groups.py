...
async def create_new_group(data: dict, me: str = Depends(auth_required)):
    ...
    for member in members: add_group_member_db(conn, group_id, member)

    version = 1
    session_key = os.urandom(16)
    print(f"[CRYPTO-DEBUG] 2. Gerada a primeira chave de sessão do grupo (Versão {version}): {session_key.hex()}")
    
    print(f"[CRYPTO-DEBUG] 3. Distribuindo a chave de sessão para os membros:")
    for member in members:
        public_pem = fetch_user_public_key(conn, member)
        if not public_pem: continue
    ...
    conn.commit()
    conn.close()
    print(f"[CRYPTO-DEBUG] ===== FIM DA CRIAÇÃO DO GRUPO =====\n")
    return {"ok": True, "id": group_id}

@router.post("/groups/{group_id}/add")
async def add_member(group_id: int, data: dict, requester: str = Depends(auth_required)):
    ...
    print(f"[CRYPTO-DEBUG] 3. Nova chave de sessão do grupo gerada: {new_session_key.hex()}")
    
    updated_members = fetch_group_members(conn, group_id)
    print(f"[CRYPTO-DEBUG] 4. Distribuindo a NOVA chave para a lista atualizada de membros: {updated_members}")
    ...
    conn.commit()
    conn.close()
    print(f"[CRYPTO-DEBUG] ===== FIM DA ADIÇÃO DE MEMBRO =====\n")
    return {"ok": True}

@router.post("/groups/{group_id}/remove")
async def remove_member(group_id: int, data: dict, requester: str = Depends(auth_required)):
    ...
    print(f"[CRYPTO-DEBUG] 4. Distribuindo a NOVA chave para os membros restantes: {remaining_members}")
    ...
    conn.commit()
    conn.close()
    print(f"[CRYPTO-DEBUG] ===== FIM DA REMOÇÃO DE MEMBRO =====\n")
    return {"ok": True}


# O restante das funções (listagem, envio e histórico) permanecem as mesmas que eu te enviei da última vez.
# Cole o restante do seu arquivo groups.py aqui para garantir que nada seja perdido.
@router.get("/groups")
async def list_user_groups(me: str = Depends(auth_required)):
    ...
    for group_id in group_ids:
        ...
        if not group_row: continue
        
        members = fetch_group_members(conn, group_id)
        
        cur.execute("SELECT MAX(key_version) as v FROM group_keys WHERE group_id = ?", (group_id,))
        version_row = cur.fetchone()
        latest_version = version_row['v'] if version_row and version_row['v'] is not None else 0
...
async def send_group_message(group_id: int, data: dict, sender: str = Depends(auth_required)):
    ...
    cur.execute("SELECT 1 FROM group_members WHERE group_id = ? AND username = ?", (group_id, sender))
    if not cur.fetchone():
        conn.close()
    ...
    cur.execute("SELECT encrypted_key FROM group_keys WHERE group_id = ? AND key_version = ? AND username = ?",
                (group_id, latest_version, sender))
    key_row = cur.fetchone()
    if not key_row:
        conn.close()
        raise HTTPException(status_code=400, detail="Não foi possível encontrar a chave de sessão.")

    sender_priv_key = fetch_user_private_key(conn, sender)
    if not sender_priv_key:
        raise HTTPException(status_code=500, detail="Chave privada do remetente não encontrada.")
    ...
    try:
        ...
        conn.close()
        raise HTTPException(status_code=500, detail=f"Erro ao descriptografar chave de sessão: {e}")
    ...
    payload = {
        "type": "group", "from": sender, "conversation_id": group_id, 
        "message": message, "msg_id": msg_id, "key_version": latest_version
    }
    if manager:
...
async def get_group_history(group_id: int, me: str = Depends(auth_required)):
    ...
    user_keys = {}
    cur.execute("SELECT key_version, encrypted_key FROM group_keys WHERE group_id = ? AND username = ?", (group_id, me))
    ...
    cur.execute("SELECT * FROM group_messages WHERE group_id = ? ORDER BY timestamp ASC", (group_id,))
    messages = []
