...
async def send_dm(data: dict, sender: str = Depends(auth_required)):
    ...
    if not to or not message:
        raise HTTPException(status_code=400, detail="Destinatário e mensagem são obrigatórios")
    ...
    try:
        ...
        print(f"[CRYPTO-DEBUG] 1. Chave de sessão única (Blowfish) criada: {session_key.hex()}")
        ...
        print(f"[CRYPTO-DEBUG] 2. Mensagem '{message}' criptografada com a chave de sessão. Ciphertext: {ciphertext_b64[:30]}...")
        ...
        print(f"[CRYPTO-DEBUG] 3. Chave de sessão criptografada para o destinatário '{to}' usando a chave pública dele.")
        print(f"[CRYPTO-DEBUG] 4. Chave de sessão criptografada para o remetente '{sender}' usando a própria chave pública.")
        ...
        cur.execute(
            ...
            (sender, to, ciphertext_b64, recipient_encrypted_b64, sender_encrypted_b64, iv.hex())
        )
        conn.commit()
        msg_id = cur.lastrowid
        print(f"[CRYPTO-DEBUG] 5. Mensagem e chaves criptografadas salvas no BD com ID: {msg_id}")

    finally:
        if conn:
            conn.close()

    payload = { "type": "dm", "from": sender, "to": to, "message": message, "msg_id": msg_id }
    if manager:
        await manager.send_to_user(to, payload)
        await manager.send_to_user(sender, payload)
    print(f"[CRYPTO-DEBUG] ===== FIM DO ENVIO DE MENSAGEM =====\n")
...
async def history(peer: str, me: str = Depends(auth_required)):
    ...
    cur.execute("SELECT * FROM messages WHERE (sender_username = ? AND receiver_username = ?) OR (sender_username = ? AND receiver_username = ?) ORDER BY timestamp ASC", (me, peer, peer, me))
    rows = cur.fetchall()
    
    out = []
    my_priv_key = fetch_user_private_key(conn, me)
    print(f"[CRYPTO-DEBUG] 1. Buscando chave privada de '{me}' para descriptografar mensagens.")
    ...
    for r in rows:
        ...
        if r["sender_username"] == me:
            ...
            print(f"[CRYPTO-DEBUG] 2. Descriptografando mensagem ENVIADA (ID: {r['id']}). Usando a 'sender_encrypted_session_key'.")
        else:
            encrypted_session_key_to_use = r["encrypted_session_key"]
            print(f"[CRYPTO-DEBUG] 2. Descriptografando mensagem RECEBIDA de '{r['sender_username']}' (ID: {r['id']}). Usando a 'encrypted_session_key'.")
        ...
        if my_priv_key and encrypted_session_key_to_use:
            try:
                ...
                print(f"[CRYPTO-DEBUG] 3. Chave de sessão descriptografada com sucesso: {session_key.hex()}")
        ...
        out.append({ "id": r["id"], "sender_username": r["sender_username"], "plaintext": plaintext, "timestamp": r["timestamp"] })
        
    conn.close()
    print(f"[CRYPTO-DEBUG] ===== FIM DO CARREGAMENTO DO HISTÓRICO =====\n")
    return out